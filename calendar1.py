...
T.insert(tk.END, "")
T.pack()
# --- อันนี้คือฟังก์ชัน on_date_select ที่ถูกต้อง (ให้แสดงวันที่ใน Text ด้วย)
def on_date_select(event):
    date = cal.get_date()
    dt = datetime.strptime(date, "%d/%m/%Y")
    thai_date = format_thai_date(dt)

    # ล้างข้อความเก่าก่อนแทรกใหม่ (ถ้าต้องการ)
    T.delete("1.0", tk.END)

    # แสดงวันที่ใน Text
    T.insert(tk.END, f"\n[{thai_date}] เขียนโน้ตของคุณที่นี่...\n")

    # อัปเดต label ด้วย (ต่อจากอันเดิม)
    label_thai.config(text=f"คุณเลือก: {thai_date}")
    cal.bind("<<CalendarSelected>>", on_date_select)
